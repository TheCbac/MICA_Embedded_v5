...
//        theta = atan2(yVal,zVal);
//        uint8 thetaMsb = MASK_BYTE_ONE & (theta >> BITS_ONE_BYTE);
//        UART_UartPutChar(thetaMsb);
...
//        BMX055_Gyr_Read(gyrData, channels);
//        
//        /* Get the X value of the gyroscope */
//        uint16 xValG = gyrData[BMX055_CHANNEL_INDEX_DATA + BMX055_CHANNEL_INDEX_X];
//        uint8 xMsbG = MASK_BYTE_ONE & (xValG >> BITS_ONE_BYTE);
//        uint8 xLsbG = MASK_BYTE_ONE & xValG;
//        /* Get the Y value of the gyroscope */
//        uint16 yValG = gyrData[BMX055_CHANNEL_INDEX_DATA + BMX055_CHANNEL_INDEX_Y];
//        uint8 yMsbG = MASK_BYTE_ONE & (yValG >> BITS_ONE_BYTE);
//        uint8 yLsbG = MASK_BYTE_ONE & yValG;
//        /* Get the Z value of the gyroscope */
//        uint16 zValG = gyrData[BMX055_CHANNEL_INDEX_DATA + BMX055_CHANNEL_INDEX_Z];
//        uint8 zMsbG = MASK_BYTE_ONE & (zValG >> BITS_ONE_BYTE);
//        uint8 zLsbG = MASK_BYTE_ONE & zValG;
//        
//        /* Debug over the UART (if enabled) */
//        uartApi_log("X:0x%x%x Y:0x%x%x Z:0x%x%x\r\n", xMsbG, xLsbG, yMsbG, yLsbG, zMsbG, zLsbG);
        /* write out the values */
//        UART_UartPutChar(xMsbG);
//        UART_UartPutChar(xLsbG);
//        UART_UartPutChar(yMsbG);
//        UART_UartPutChar(yLsbG);
//        UART_UartPutChar(zMsbG);
//        UART_UartPutChar(zLsbG);
